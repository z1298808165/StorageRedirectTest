...
`test_case=all` 会运行查询、创建、读取、写入和文件 API 的主要路径，但不会自动执行 MediaStore 删除或缩略图用例。它会创建 `srt_*` 开头的测试媒体文件，建议在测试机或可清理环境中运行。
...
- 启用 `read_only_paths`，验证可读但拒绝写入、删除、mkdir、rename。
